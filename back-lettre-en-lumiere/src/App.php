...
class App
{
    ...
    {
        $query = "SELECT DISTINCT *,  seq_title, seq_description  FROM l_EXERCICES exo 
        INNER JOIN l_SEQUENCES seq on exo.sequence_id = seq.sequence_id";
    ...
    {
        //         SELECT  *,sta_name, sta_description FROM l_SEQUENCES seq INNER JOIN l_STAGES sta on seq.stage_id = sta.stage_id
// WHERE sta.stage_id = seq.stage_id 

        $query = "SELECT *, sta_name, sta_description FROM l_SEQUENCES seq 
                  INNER JOIN l_STAGES sta on seq.stage_id = sta.stage_id
                  WHERE sta.stage_id = seq.stage_id";

        $statement = $this->pdo->query($query);
        $sequences = [];
        while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
            $sequences[] = new M_Sequence(
                $row['sequence_id'],
                $row['seq_title'],
                $row['stage_id'],
                $row['seq_description'],
