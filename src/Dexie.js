...
db.transaction('rw', 'users', 'stages', 'sequences', 'exercises', 'userProgress', 'test', async () => {
    const countStages = await db.stages.count();
    const countUsers = await db.users.count();
    const countSequences = await db.sequences.count();
    const countExercises = await db.exercises.count();
    ...
    if (countSequences === 0) {
        await db.sequences.bulkAdd([
            ...
            { title: 'o-u-e', stageId: 1, description: 'Les voyelles', content: ['o, u, é'], medal: null },
